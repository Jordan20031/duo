...
function takeCardActions() {
    ...
    if (currentPlayer == 0) {
        console.log("You have no cards to place");
        ...
        setTimeout(() => {
            ...
            if (!isValidCard(takenCard)) {
                setTimeout(() => {
                    // If the taken card is invalid, give another random card and pass the turn
                    console.log(`Player ${currentPlayer + 1} has no cards to place`);
                    giveRandomCard(currentPlayer);
                    turn += direction;
                    handleTurn();
                    return;
                }, 1500)
...
function giveRandomCard(playerIndex) {
    // Retrieve a random card from the deck
    const randomIndex = Math.floor(Math.random() * deck.length);
    const randomCard = deck.splice(randomIndex, 1)[0];
    // Add the random card to the player's hand
    players[playerIndex].push(randomCard);
    // Reorganize the player's cards
    players[playerIndex] = organizeCards(players[playerIndex]);
...
function handleClick(cardObject) {
    ...
    if (!isValidCard(selectedCard)) {
        console.log("Invalid card! You need to draw a card.");
        turnDeckCard(document.getElementById("deck"));
        return;
    }
...
function turnDeckCard(deckObject) {
    ...
    if (!isValidCard(takenCard)) {
        console.log("Cannot place this card");
        setTimeout(() => {
            deckObject.classList.toggle("turned");
            renderUserCards();
            turn += direction;
...
function deplacerMouche() {
    const screenWidth = window.innerWidth;
    const screenHeight = window.innerHeight;

    
    const randomX = Math.random() * (screenWidth - mouche.offsetWidth);
    const randomY = Math.random() * (screenHeight - mouche.offsetHeight);

   
    const deltaX = randomX - mouche.offsetLeft;
    const deltaY = randomY - mouche.offsetTop;


    mouche.style.transform = `translate(${deltaX}px, ${deltaY}px)`;

    
    mouche.style.left = randomX + "px";
    mouche.style.top = randomY + "px";
}


setInterval(deplacerMouche, 5000); 
...
function renderDeck() {
    let deckObject = document.getElementById("deck");
    let symbolHTML;
    let carte = deck[0]; // Remplacer 'carta' par 'carte'
